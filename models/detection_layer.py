...
from models.utils import unique1d, intersect1d, parse_image_meta
from models.rpn import apply_box_deltas
import numpy as np
import torchvision
...
def detection_target_layer(proposals_list, gt_class_ids_list, gt_boxes_list, gt_masks_list, config):
    ...
    rois_list, roi_gt_class_ids_list, deltas_list, masks_list = [], [], [], []
    for proposals, gt_class_ids, gt_boxes, gt_masks in zip(proposals_list, gt_class_ids_list, gt_boxes_list,
                                                           gt_masks_list):
        if torch.nonzero(gt_class_ids < 0).size()[0]:
            crowd_ix = gt_class_ids < 0
            crowd_ix = torch.nonzero(crowd_ix)
            crowd_ix = crowd_ix[:, 0]
        ...
        if positive_indices.size()[0] != 0:
            ...
            boxes_w_ix = torch.cat((box_ids.unsqueeze(1), boxes), dim=1)
            roi_masks = roi_masks.unsqueeze(1)
            masks = Variable(torchvision.ops.roi_align(roi_masks, boxes_w_ix,
                                                       (config.MASK_SHAPE[0], config.MASK_SHAPE[1]), sampling_ratio=0,
                                                       aligned=True))
    ...
    return rois_list, roi_gt_class_ids_list, deltas_list, masks_list  # rois, roi_gt_class_ids, deltas, masks
...
def refine_detections(rois, probs, deltas, window, config):
    ...
    for i, class_id in enumerate(unique1d(pre_nms_class_ids)):
        ...
        class_keep = torchvision.ops.nms(ix_rois, ix_scores, iou_threshold=config.DETECTION_NMS_THRESHOLD)
        # Map indicies
